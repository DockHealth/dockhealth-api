...
test('get events range', async () => {
  ...
  const events = await getEventsRange(headers, start, end)
...
test('get event delivery range', async () => {
  ...
  const end = dayjs().utc().toISOString()
  const start = dayjs().utc().subtract(1, 'day').toISOString()
  const attempts = await getEventDeliveryAttemptsRange(headers, start, end)
...
test('lifecycle', async () => {
  ...
  try {
    ...
    const beforeEvents = await getLatestEvents(headers)
    const beforeAttempts = await getLatestAttempts(headers)
    ...
    let afterEvents = await getLatestEvents(headers)
    expect(afterEvents.length).toBeGreaterThan(beforeEvents.length)
    let afterAttempts = await getLatestAttempts(headers)
    ...
    // See that we have one more event and event delivery attempt than before.
    afterEvents = await getLatestEvents(headers)
    expect(afterEvents.length).toBeGreaterThan(beforeEvents.length)
    afterAttempts = await getLatestAttempts(headers)
    expect(afterAttempts.length).toBeGreaterThan(beforeAttempts.length)
  } catch (error) {
    ...
    await deleteWebhook(headers, webhook)
...
const getLatestEvents = async (headers) => {
  const endTs = dayjs().utc().toISOString()
  const startTs = dayjs().utc().subtract(1, 'minute').toISOString()
  return await getEventsRange(headers, startTs, endTs)
}

const getLatestAttempts = async (headers) => {
  const endTs = dayjs().utc().toISOString()
  const startTs = dayjs().utc().subtract(1, 'minute').toISOString()
  return await getEventDeliveryAttemptsRange(headers, startTs, endTs)
}

const getEventsRange = async (headers, startTs, endTs) => {
  const url = encodeURI(`/api/v1/event?organization=${shared.organizationIdentifier}&startTs=${startTs}&endTs=${endTs}`)
...
const getEventDeliveryAttemptsRange = async (headers, startTs, endTs) => {
  const url = encodeURI(`/api/v1/event/delivery?organization=${shared.organizationIdentifier}&startTs=${startTs}&endTs=${endTs}`)
  return await request
    .get(url)
    .set(headers)
    .expect(200)
...
const deleteWebhook = async (headers, webhook) => {
  if (webhook) {
    await request
      .delete('/api/v1/webhook/' + webhook.id)
      .set(headers)
      .expect(200)
      .then(res => {
